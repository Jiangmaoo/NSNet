...
parser.add_argument('--input_dir', default='/home/jiangmao/dataset/DndRgbMat/', type=str, help='Directory of validation images')
parser.add_argument('--result_dir', default='/home/jiangmao/code/NSNet/one_128/results/DND/', type=str, help='Directory for results')
parser.add_argument('--weights', default='/home/jiangmao/code/NSNet/one_128/results/Training-Results/Best.pkl', type=str, help='Path to weights')
...
model_restoration = build_net('test')

#utils.load_checkpoint(model_restoration,args.weights)
state_dict = torch.load(args.weights)
model_restoration.load_state_dict(state_dict['model'])
...
with torch.no_grad():
    for i in tqdm(range(50)):
        ...
        for k in range(20):
            ...
            #restored_patch = model_restoration.test(noisy_patch)[2]
            restored_patch = model_restoration.module.test(noisy_patch)[2] if hasattr(model_restoration, 'module') else model_restoration.test(noisy_patch)[2]
            # print(restored_patch[0].shape)
            # print(type(restored_patch[0]))
            # restored_patch = torch.clamp(restored_patch[0], 0, 1).cpu().detach().permute(0, 2, 3, 1).squeeze(0).numpy()
            restored_patch=torch.clamp(restored_patch[0], 0, 1).cpu().detach().squeeze(0).numpy()
