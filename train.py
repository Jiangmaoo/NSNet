...
from utils import Adder, Timer, CharbonnierLoss, mkdir, KLLoss
...
def _train(model, args):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, betas=(0.9, 0.9), eps=1e-8)
    dataloader = train_dataloader(args.data_dir, args.batch_size, args.num_worker)
    max_iter = len(dataloader)

    warmup_epochs = 3
    scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.num_epoch - warmup_epochs,
                                                                  eta_min=1e-6)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            optimizer.zero_grad()
            pred_img, pred_gt = model(input_img, label_img)
            ...
            # 重建的噪声图像损失对比  加了kl损失1123
            input_img2 = F.interpolate(input_img, scale_factor=0.5, mode='bilinear')
            input_img4 = F.interpolate(input_img, scale_factor=0.25, mode='bilinear')
            l1_n = criterion_kl(pred_gt[0], input_img4)
            l2_n = criterion_kl(pred_gt[1], input_img2)
            l3_n = criterion_kl(pred_gt[2], input_img)
            loss_content_n = l1_n + l2_n + l3_n
